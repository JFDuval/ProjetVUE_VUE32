...
void __ISR(_TIMER_3_VECTOR, ipl3) Fct_T3_IntHandler (void)//20kHz
{
...
void Fct_PWM_init(unsigned char ucMotor)
{
    ...
    if (ucMotor & 1)
    {
        ...
        function_table[ucFunctTableSize++] = &Fct_pwm_motor_A;
     ...
     if(ucMotor & 2)
    {
        ...
        function_table[ucFunctTableSize++] = &Fct_pwm_motor_B;
    }
    T3CONbits.ON =1;               //Activate Timer3
}

void Fct_pwm_motor_A()
{
    // do something with motor A
    // think about current in the motor depending on the motor type

}
void Fct_pwm_motor_B()
{
    // do something with motor B
}


void Fct_motor_control( MOTOR *sel_motor,unsigned char dir, unsigned char speed)
{
    (*sel_motor).IN1 = 0;
    (*sel_motor).IN2 = 0;
...
void Fct_Lock_Init(void)    //Must be called after defining the lock state
{
    function_table[ucFunctTableSize++] = &Fct_Lock;
